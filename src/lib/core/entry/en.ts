...
const createLookupWordsEn = (rawSourceStr: string, withCapitalized = false, mustIncludeOriginalText = false): string[] => {
  const replacedSourceStr = rawSourceStr.replace(RE_UNNECESSARY_CHARACTERS, "").replace(RE_SLASH, " / ");
  const sourceStr = text.dealWithHyphens(replacedSourceStr, (code) => {
    return rule.doLetters(code) !== undefined;
  });
  ...
  if (mustIncludeOriginalText && !lookupWords.has(rawSourceStr)) {
...
const generateTitledExpressions = (words: string[]) => {
  if (!(words?.length >= 1)) {
    return [];
  }
  const result: string[] = [];
...
const processSourceString = (sourceStr: string) => {
...
const dealWithFirstWordHyphen = (theFirstWord: string) => {
...
const divideIntoTwoWords = (str: string) => {
  const result: string[] = [];
...
const cutDuplicatedLetters = (str: string) => {
  let prevCode = 0;
  let count = 0;
...
const isValidCharacter = (ch: number) => {
  const letter = rule.doLetters(ch);
  return letter !== undefined;
};

const createWordsList = (str: string) => {
  if (!str) {
    return [];
  }
...
const createLinkedWords = (words: string[], isAllLower: boolean) => {
 ...
 */
const createLinkedWordList = (arr: string[], allLowercase: boolean, minWordNum = 1) => {
 ...
 */
const parseFirstWord = (sourceStr: string, ignoreLowerCase: boolean, minLength = 3) => {
  if (!sourceStr) {
    return [];
  }
  const wordList = new UniqList<string>((a) => a.length >= minLength);
  ...
  for (let i = 0; i < strList.length; i++) {
    ...
    if (arr.length >= 2) {
      ...
      // Add a postfix
      const last = arr.at(-1);
      if (last && isHyphenLikeCharacter(sourceStr, sourceStr.length - last.length - 1)) {
        wordList.push("-" + last);
      }
    }
  }
...
const isHyphenLikeCharacter = (sourceStr: string, position: number) => {
...
const fetchQuotedStrings = (str: string) => {
  const result: string[] = [];
